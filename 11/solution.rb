...
class Solution < Base
  ...
  def parse_input
    ...
    input_lines.each do |line|
      ...
      monkey.items = Regexp.last_match(1).split(', ').map(&:to_i) unless line.match(/Starting items: (.+)/).nil?
    ...
    @modulo = monkeys.map(&:divisible).inject(:*)
  end
